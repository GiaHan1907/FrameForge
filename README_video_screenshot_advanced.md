...
Trong giao diện (bố cục v0.1.39), toàn bộ cấu hình nằm trong tab **⚙️ Xử lý video**: wizard 4 bước + 4 card tóm tắt, bên dưới là 4 expander cấu hình khớp từng bước — `01 · Nguồn video` (upload nhiều video mp4/mov/mkv/avi/webm/m4v/ts/mts), `02 · Cách chọn frame`, `03 · Chất lượng & tốc độ`, `04 · Đầu ra`. Sidebar chỉ còn thương hiệu và nút **🔍 Tìm ảnh theo địa điểm**. Nội dung chính gồm 3 tab: **⬇️ Tải video công khai** (yt-dlp), **⚙️ Xử lý video** (wizard 4 bước + 4 expander cấu hình + nút **Bắt đầu xử lý** + preview workspace) và **📁 Cài đặt & Lịch sử** (cập nhật, preset cá nhân, lịch sử job). Có bốn chế độ chọn frame: **Best frame per scene**, **Scene detection**, **Mỗi N giây** và **Đúng N frame**; ngưỡng scene, độ nét, dHash, motion blur, định dạng và kích thước ảnh đều chỉnh được trong các expander thu gọn.
