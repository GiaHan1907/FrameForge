...
class StreamlitDirectoryStateTests(unittest.TestCase):
    ...
    def test_presets_and_progress_telemetry_are_present(self) -> None:
        ...
        self.assertIn('target_count_after_filter', self.source)
        self.assertIn('Ép đủ số ảnh yêu cầu (fallback cuối)', self.source)
        self.assertIn('Phân bố screenshot dự kiến', self.source)
        self.assertIn('target_count_after_filter', self.source)
        self.assertIn('min_free_ram_gb', self.source)
        self.assertIn('shortfall', self.source)
        engine_source = (ROOT / 'video_screenshot_advanced.py').read_text(encoding='utf-8')
        self.assertIn('force_fill_target', engine_source)
        self.assertIn('force_fill_shortfall', engine_source)
        self.assertIn('manifest_path', engine_source)
        self.assertIn('verify_video_manifest', engine_source)
        self.assertIn('resource_wait', engine_source)
        self.assertIn('InsufficientResources', self.source)
        ...
        self.assertIn('sticky-summary', self.source)
        self.assertIn('def render_queue_dashboard', self.source)
        self.assertIn('def render_resource_meter', self.source)
        self.assertIn('error_actions', self.source)
        self.assertIn('PERSONAL_PRESET_KEYS', self.source)
        self.assertIn('def save_personal_preset', self.source)
        self.assertIn('def append_job_history', self.source)
        self.assertIn('def export_ui_config', self.source)
        self.assertIn('def import_ui_config', self.source)
        self.assertIn('Lịch sử job', self.source)
        self.assertIn('disabled=bool(validation_errors)', self.source)
        ...
        self.assertIn('def preview_frame_at', self.source)
        self.assertIn('def preview_scene_timeline', self.source)
        self.assertIn('Preview workspace', self.source)
        self.assertIn('Mốc preview', self.source)
        self.assertIn('Frame gallery', self.source)
        self.assertIn('preview_col, crop_preview_col = st.columns([1.15, 1]', self.source)
        ...
        self.assertIn('processing_signature(resume_args_preview)', self.source)
        self.assertIn('Phân tích nhanh scene thật', self.source)
